...
tb24_true <- read_csv("~/Desktop/thesis/tibet-toolkit/python_code/24_km.csv")
...
#lowest aic so far...
fit5 <- arima(ts.log, order=c(6,0,0))
print(fit5)
tsdiag(fit5)
myRes5 <- residuals(fit5)



#autofit doesn't quite work as well as manually searching
autotpts = auto.arima(tpts, seasonal=TRUE)
print(autotpts)
tsdiag(autotpts)
...
par(mfrow=c(1,1))
qqnorm(myRes5, main = "Normal Q-Q Plot", xlab = "Theoretical Quantiles", ylab = "Sample Quantiles")
