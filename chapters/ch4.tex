...
\begin{itemize}
    ...
    The reason for having this screen is to get the location of the users that are using the application for analytics purpose. This screen makes use of Apple's Core Location framework which allows developers to obtain current geographic location of the device. In this screen, user has two possible choices for entering the location, the user can enter the area physically or can simply tap on pickup current area which will eventually get the present location and display it in text area. Despite everything the user can still alter the area if needed.
...
\begin{itemize}
    ...
    This screen is to make this process more secure by asking users year of birth. It must match the year of birth they entered in registering via email process. If this matches, then user is allowed to change the password.
